...
if(isset($_FILES['fileToUpload'])){
    ...
    if(empty($errors)==true) {
     ...
     if ($res){
        ...
        $articleNumber = array();
        $articleImage = array();
        foreach($res['article'] as $imgname){
            //$key = $row;
            //$tmpkey = explode( '-', trim($key), 2)[0];
            //$tmpkey = explode('_', $tmpkey, 2)[0];
            if( !array_key_exists($imgname, $articleNumber) ) $articleNumber[$imgname] = '';
        }
        // echo '<pre>';
        ...
        foreach( $articleNumber as $imgname => $val){
            foreach($res['img'] as $imgpath){
                if( strpos($imgpath, $imgname) !== false){
                    $articleNumber[$imgname] = $articleNumber[$imgname] ? $articleNumber[$imgname].','.$imgpath : $imgpath;
                }    
            }
